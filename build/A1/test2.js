...
class toJSON extends stream_1.Transform {
    constructor(opt) {
        ...
        this.stateLogger = fs.createWriteStream(OUTPUT_PATH);
    }
    set accState(x = initState) {
        this.stateLogger.write(x, e => console.log(`toJSON setState Error`));
        //this.accState = x
    }
    _transform(chunk, encoding, callback) {
        ...
        try {
            ...
            const res = match ? match.flat().reduce(defragStream, this.accState) : false;
...
const jTx = new toJSON({ writableObjectMode: false, readableObjectMode: true });
...
stream_1.pipeline(c$, jTx, toString, process.stdout, (e) => {
    if (e)
        console.log(e);
    else
        console.log("success");
